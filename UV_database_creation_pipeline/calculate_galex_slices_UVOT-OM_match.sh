...
for f in $( ls $datapath/Galex/GUVCat_*.csv.gz); do
    ...
    if [ "${COUNT}" -gt 0 ]; then
	    ...
	    if [ $j -gt 0 ]; then
		    ...
		    stilts tcat ifmt=csv in="$datapath/Galex/match_Galex_UVOT_OM.csv \
		    ...
		    mv $datapath/Galex/temp.csv $datapath/Galex/match_Galex_UVOT_OM.csv
	    else
		    mv $datapath/Galex/temp_match.csv $datapath/Galex/match_Galex_UVOT_OM.csv
	    fi
	    let j=j+1
