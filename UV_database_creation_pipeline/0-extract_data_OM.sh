...
j=0
# Path to the data
# Note that the existence of a sub-repository named XMM-Newton with 
# the catalogue as XMM-OM-SUSS6.2.fits is necessary.
...
omode=out ofmt=fits in="$datapath_ext"/XMM-OM-SUSS6.2.fits#1 \
out=$datapath/XMM-Newton/temp.fits

# Divide the time table in various parts in order to spare RAM for match
echo "Dividing the time table in various parts"
python Utilities/divide_time_table.py "$datapath_ext/XMM-OM-SUSS6.2.fits" "$datapath/XMM-Newton"
...
for f in $( ls $datapath/XMM-Newton/temp_part*.fits); do
    ...
    UVM2_QUALITY_FLAG UVW1_QUALITY_FLAG U_QUALITY_FLAG B_QUALITY_FLAG \
    V_QUALITY_FLAG UVW2_EXTENDED_FLAG UVM2_EXTENDED_FLAG UVW1_EXTENDED_FLAG \
    U_EXTENDED_FLAG B_EXTENDED_FLAG V_EXTENDED_FLAG MJD_START OBSID_1"' \
