...
def print_light_curve(df,TDE_path):
    ...
    for srcnum in df['SRCNUM_CUV'].unique():
        ...
        plt.title("Source (RA DEC) : " + "{0:.5f}".format(ra) + " " + \
                  "{0:.5f}".format(dec))
        fig.savefig(roots_tfm + TDE_path +'/All bands/SCRNUM_CUV_'+str(srcnum)+'.png',dpi=1200)
...
def print_source_infos(df_results,TDE_path):
    ...
    df_results_sources = df_results[['SRCNUM_CUV','RA','DEC']].drop_duplicates('SRCNUM_CUV').sort_values(['SRCNUM_CUV']).reset_index(drop=True)
    
    # Addition of galex observation date
    df_results_sources['GALEX_OBS_DATE'] = np.nan
    ...
    for index in df_test_galex.index:
       result= subprocess.run(["python","client_galex.py", 
                               str(round(df_results.loc[index,'RA'],5)), 
                               str(round(df_results.loc[index,'DEC'],5))], 
...
#                       capture_output=False,text=True)
    ...
    df_results_sources['ESASky_link'] = 'https://sky.esa.int/esasky/?target='+str(df_results_sources['RA'])+' '+str(df_results_sources['DEC'])+\
            '&hips=GALEX+GR6%2F7+AIS+color&fov=0.08125700464842057&projection=SIN&cooframe=J2000&sci=true&lang=en'
    
    # Write results
    df_results_sources.to_csv(roots_tfm + TDE_path + '/All bands/TDE_candidates_infos.csv')
...
def get_list_best_CUV(TDE_path):
    ...
    for item in contents:
        ...
        if os.path.isfile(item_path) and len(item.split('_'))>2:
...
if __name__ == "__main__":
    ...
    TDE_path = '/TDE/Candidates/Ratio higher than 4 and rest lower than 2 (UVW2 and UVM2), color inversion'
    ...
    print_light_curve(df_results,TDE_path)
    
    print_source_infos(df_results,TDE_path)
